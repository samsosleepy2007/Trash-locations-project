...
try{
 ...
 const admin=await fixture('admin');await admin.page.goto('http://127.0.0.1:8766/admin.html');await admin.page.getByRole('button',{name:'ตรวจรูป'}).waitFor();await admin.page.screenshot({animations:'disabled',path:'verification/activity-admin.png',fullPage:true});await admin.page.getByRole('button',{name:'ตรวจรูป'}).click();await admin.page.locator('.a-proof').waitFor();const rejectButton=admin.page.getByRole('button',{name:'ผิด · ไม่ให้คะแนน',exact:true});assert.ok(await rejectButton.isDisabled());await admin.page.getByLabel('เหตุผลกรณีไม่ผ่าน (จำเป็น)',{exact:true}).fill('รูปไม่เห็นถังแยกประเภทชัดเจน');assert.ok(!(await rejectButton.isDisabled()));await admin.page.getByRole('button',{name:'ถูกต้อง · +1 คะแนน',exact:true}).click();await admin.page.getByText('อนุมัติแล้ว เพิ่ม 1 คะแนน',{exact:true}).waitFor();assert.equal(admin.calls.filter(x=>x.path.endsWith('/activity_review')).length,1);assert.equal(admin.calls.find(x=>x.path.endsWith('/activity_review')).body.p_decision,'approved');await admin.context.close();
