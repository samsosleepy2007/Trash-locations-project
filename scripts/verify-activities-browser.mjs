...
try{
 ...
 await p.goto('http://127.0.0.1:8766/activity.html');await p.waitForSelector('.a-podium-place');assert.equal(await p.locator('.a-podium-place').count(),3);assert.equal(await p.locator('.a-waiting').count(),3);assert.ok(await p.getByRole('button',{name:'ยังไม่เปิดรับกิจกรรม'}).isDisabled());await p.screenshot({path:'verification/activity-desktop.png',fullPage:true});
 await p.setViewportSize({width:390,height:844});await p.screenshot({path:'verification/activity-mobile.png',fullPage:true});assert.ok(await p.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
 ...
 const first=await fixture('student');await first.page.goto('http://127.0.0.1:8766/activity.html');await first.page.getByRole('button',{name:'ส่งกิจกรรม',exact:true}).click();await first.page.getByLabel('ชื่อที่จะแสดง',{exact:true}).fill('ผู้เข้าร่วมทดสอบ');await first.page.getByLabel('คณะ',{exact:true}).fill('วิทยาศาสตร์');await first.page.getByLabel('สาขา',{exact:true}).fill('คอมพิวเตอร์');await first.page.getByRole('checkbox').check();await first.page.getByLabel('อัปโหลดรูปกิจกรรม').setInputFiles('image/nrru-logo.png');await first.page.locator('.a-proof-preview').waitFor();await first.page.screenshot({path:'verification/activity-submit.png'});await first.page.getByRole('button',{name:'ส่งให้ผู้ดูแลตรวจ'}).click();await first.page.locator('.a-success').waitFor();assert.equal(first.calls.filter(x=>x.path.endsWith('/activity_save_profile')).length,1);assert.equal(first.calls.filter(x=>x.path.endsWith('/activity_submit')).length,1);await first.context.close();
