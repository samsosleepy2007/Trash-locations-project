...
create or replace function activity_private.is_github_prize_url(p_value text) returns boolean
language sql
immutable
set search_path=''
as $$
  select coalesce(p_value,'') ~ '^https://raw\.githubusercontent\.com/samsosleepy2007/Trash-locations-project/main/[^[:space:]]+\.(png|jpg|jpeg|webp)$';
$$;

revoke all on function activity_private.is_github_prize_url(text) from public,anon,authenticated;
